...
- **재접속**: 학생 페이지가 `sessionStorage` 토큰으로 식별 — 끊겨도 같은 점수로 복귀.
  ...
  QR 을 다시 찍으면 탭이 새로 열려 토큰도 새로 생기므로, **같은 이름의 끊긴 플레이어를 이어받는다**.
  로비에서 끊기면(지킬 점수가 없다) 명단에서 지운다 — 안 그러면 나가거나 이름을 바꾼 학생의 옛
  이름이 유령으로 남는다.
- **이름 중복은 거절한다**: 같은 이름이 *아직 붙어 있는데* 다른 토큰으로 또 들어오면 새 플레이어를
  만들지 않고 "이미 참가 중인 이름입니다" 를 돌려준다. 실제 수업에서 옛 탭을 열어 둔 채 QR 을 다시
  찍은 학생 3명이 각각 둘·셋으로 갈려 점수를 잃었다(결과표에 0점 줄이 남았다). 이름 비교는 앞뒤
  공백과 대소문자를 무시한다. 같은 토큰(같은 탭 새로고침)은 언제나 그대로 들여보낸다.
  끊긴 연결을 빨리 알아채야 본인이 다시 들어올 수 있으므로 `pongWait` 은 30초로 둔다.
...
- 참가자 페이지 비속어 필터(닉네임 중복은 처리됨 — 이미 참가 중이면 거절)
