...
`~/.claude.json` / Cursor)를 훑어 **이미 있는 설정 파일에만** 등록하고, 하나도 없으면 가장 흔한
Claude Desktop 자리에 만든다. 목록에 없는 프로그램은 `mcp-install <경로>` 로 알려 주면 그 경로를
데이터 폴더의 `mcp-clients.json` 에 기억해 함께 관리한다.
